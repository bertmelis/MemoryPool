...
Variable::Variable(size_t nrBlocks, size_t blocksize)
: _normBlocksize(0)
, _buffer(nullptr)
, _head(nullptr)
#ifdef MEMPOL_DEBUG
...
#endif
  {
  _normBlocksize = blocksize / sizeof(BlockHeader) + ((blocksize % sizeof(BlockHeader)) ? 1 : 0);
  size_t nrBlocksToAlloc = nrBlocks * (_normBlocksize + 1);
  _buffer = reinterpret_cast<unsigned char*>(::malloc(nrBlocksToAlloc * sizeof(BlockHeader)));
  if (!_buffer) {
    assert(!"Buffer could not be allocated");
    abort();  // also abort if assert is erased by NDEBUG macro
  }
...
void* Variable::malloc(size_t size) {
